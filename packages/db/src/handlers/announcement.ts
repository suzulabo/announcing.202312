...
const removeNullish = <T extends Record<string, unknown>>(o: OptionalWithNullish<T>): T => {
  const n: Partial<T> = {};
...
const writeAnnouncement = async (
  ...
  if (!updateAnnouncementId) {
    const v = removeNullish<Announcement>({
      id,
      headerImage,
      title,
      body,
    ...
    if (index >= 0) {
      ...
      announcements[index] = removeNullish<Announcement>({
        id,
        headerImage,
        title,
        body,
